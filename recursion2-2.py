...
def splitArrayHelper(a,b,c,idx):
    ...
    if (lo==hi):
        return False
    elif sum(b)==sum(nums)/2:
        return True
    else:
        c=b[:]
        b.append(nums[lo])
        return (splitArrayHelper(nums,b,lo+1) or splitArrayHelper(nums,c,lo+1))'''
...
def splitOdd10Helper(nums,b,lo):
    hi=len(nums)
    if (lo==hi):
        return False
    elif sum(b)%10==0 and (sum(nums)-sum(b))%2==1:
        return True
    else:
        c=b[:]
        b.append(nums[lo])
        return (splitOdd10Helper(nums,b,lo+1) or splitOdd10Helper(nums,c,lo+1))


# Given an array of ints, is it possible to divide the ints
# into two groups, so that the sum of the two groups is the
# same, with t:hese constraints: all the values that are
...
def split53(nums):
    b=[]
    return split53Helper(nums,b,0)
def split53Helper(nums,b,lo):
    hi=len(nums)
    if (lo==hi):
        if sum(b)==sum(nums)/2:
            return True
        ...
        if nums[lo]%5==0:
            c.append(nums[lo])
            b.append(nums[lo])
        elif nums[lo]%3==0 and nums[lo]%5!=0:
            pass
        else:
            b.append(nums[lo])
        return (split53Helper(nums,b,lo+1) or split53Helper(nums,c,lo+1))
