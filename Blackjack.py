...
class Player (object):
  ...
  def __str__ (self):
    ...
    for i in range(len(self.cards)):
        strng=strng+str(self.cards[i])+' '
    strng=strng+'- '+str(self.get_points())+' points'
    return strng
  ...
  def play (self):
    ...
    for i in self.player_list:
        if (self.dealer.has_blackjack):
          print ('Player ' + str(count+1)+' wins')
          count=count+1
          continue
        elif (player_points[count] > 21):
          print ('Player ' + str(count+1)+' loses')
          count=count+1
          continue
        elif (i.has_blackjack() or dealer_points > 21):
          print ('Player ' + str(count+1)+' wins')
          count=count+1
          continue
        elif (dealer_points <= 21) and (dealer_points >player_points[count]):
          print ('Player ' + str(count+1)+' loses')
          count=count+1
          continue
        elif (dealer_points < player_points[count] and player_points[count] <= 21):
          print ('Player ' + str(count+1)+' wins')
          count=count+1
          continue
        elif (dealer_points == max(player_points)):
          print ('Player ' + str(count+1)+' ties')
          count=count+1
          continue
